...
export async function runFlow(flowName, payload) {
  ...
  try {
    ...
    await saveLog(payload.pengajuanId, flowName, 'error', 'ERROR', error.message, Date.now() - startTime)
    throw error
  }
}
...
async function flowEksekusi(payload) {
  ...
  await logStep(pengajuanId, 'flow-eksekusi', 'kirim-email-guru', async () => {
    await kirimEmail({
      ...
      attachments: suratUrl ? [{ filename: `surat_cuti_${pengajuanId}.pdf`, path: suratUrl }] : []
    })
  })
  ...
  const html = `
    ...
    <body>
      ...
      <div class="ttd">
        ...
        <p>Kepala Sekolah,</p>
        <div class="garis">
          <p><strong>Drs. Ahmad Fauzi, M.Pd.</strong></p>
          <p>NIP. ...</p>
        </div>
      </div>
    </body>
